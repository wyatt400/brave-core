...
FilterListSubscriptionInfo BuildInfoFromDict(const GURL& list_url, const base::Value* dict) {
  ...
  double last_update_attempt;
  last_update_attempt = *dict->FindDoubleKey("last_update_attempt");
...
void AdBlockSubscriptionService::ReloadFilters() {
  ...
  base::FilePath list_location = DirForCustomSubscription(list_url_).AppendASCII(kCustomSubscriptionListText);
...
std::unique_ptr<AdBlockSubscriptionService> AdBlockSubscriptionServiceFactory(
    const FilterListSubscriptionInfo& cached_info,
    brave_component_updater::BraveComponent::Delegate* delegate) {
  auto service = std::make_unique<AdBlockSubscriptionService>(cached_info, delegate);
